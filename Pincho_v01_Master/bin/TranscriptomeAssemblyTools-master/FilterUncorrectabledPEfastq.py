...
##Editted by Randy Ortiz: renamed output files "corrected_", gz output 
...
if __name__=="__main__": 
    ...
    r1out=open('corrected_un_%s' % basename(opts.leftreads).replace('.gz',''),'w')
    r2out=open('corrected_un_%s' % basename(opts.rightreads).replace('.gz','') ,'w')
    ...
    unfix_log=open('corrected_%s.log' % opts.id,'w')
